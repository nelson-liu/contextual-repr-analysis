...
then
    ...
    echo "Please call the script with: script [task_info_path] [config_dir] [contextualizer_prefix] [name_prefix] [num_layers]"
    exit 1
fi
...
while IFS='' read -r line || [[ -n "$line" ]]; do
    ...
    for ((layer_num=0; layer_num<$NUM_LAYERS; layer_num++)); do python scripts/evaluate_with_beaker.py "${CONFIG_DIR}"/"${task_name}".json --layer-num ${layer_num} --source "contexteval_data:./data" --source "contexteval_contextualizers_${CONTEXTUALIZER_PREFIX}_${task_contextualizer_name}:./contextualizers/${CONTEXTUALIZER_PREFIX}/${task_contextualizer_filename}" --gpu-count 1 --desc "${NAME_PREFIX} on ${task_name} task with layer ${layer_num}" --name "${NAME_PREFIX}_${task_name}_layer_${layer_num}"; done
done < "${TASK_INFO_PATH}"
