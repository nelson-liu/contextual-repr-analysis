...
class TestConstituencyParentPrediction(ModelTestCase):
    def setUp(self):
        ...
        self.set_up_model(self.FIXTURES_ROOT / 'tasks' / 'constituency_ancestor_prediction' / 'experiment.json',
    ...
    def test_simple_tagger_with_contextualizer_can_train_save_and_load(self):
        self.ensure_model_can_train_save_and_load(
            self.FIXTURES_ROOT / 'tasks' / 'constituency_ancestor_prediction' /
    ...
    def test_no_input_representations_throws_error(self):
        broken_param_file = (self.FIXTURES_ROOT / 'tasks' / 'constituency_ancestor_prediction' /
                             'broken_experiments' / 'no_contextualizer.json')
        with pytest.raises(ConfigurationError):
            self.ensure_model_can_train_save_and_load(broken_param_file)


class TestConstituencyGrandparentPrediction(ModelTestCase):
    def setUp(self):
        super(TestConstituencyGrandparentPrediction, self).setUp()
        self.set_up_model(self.FIXTURES_ROOT / 'tasks' / 'constituency_ancestor_prediction' /
                          'grandparent_experiment.json',
                          self.FIXTURES_ROOT / 'data' / 'syntactic_constituency' / 'wsj.txt')
...
class TestConstituencyGreatgrandparentPrediction(ModelTestCase):
    def setUp(self):
        super(TestConstituencyGreatgrandparentPrediction, self).setUp()
        self.set_up_model(self.FIXTURES_ROOT / 'tasks' / 'constituency_ancestor_prediction' /
                          'greatgrandparent_experiment.json',
                          self.FIXTURES_ROOT / 'data' / 'syntactic_constituency' / 'wsj.txt')
