...
class TestConll2003NER(ModelTestCase):
    def setUp(self):
        super(TestConll2003NER, self).setUp()
        self.set_up_model(self.FIXTURES_ROOT / 'tasks' / 'ner' / 'experiment.json',
    ...
    def test_simple_tagger_with_contextualizer_can_train_save_and_load(self):
        self.ensure_model_can_train_save_and_load(
            self.FIXTURES_ROOT / 'tasks' / 'ner' /
            'experiment_model_contextualizer.json')

    def test_simple_crf_tagger_can_train_save_and_load(self):
        self.ensure_model_can_train_save_and_load(
            self.FIXTURES_ROOT / 'tasks' / 'ner' /
            'experiment_crf.json')
    ...
    def test_no_input_representations_throws_error(self):
        broken_param_file = (self.FIXTURES_ROOT / 'tasks' / 'ner' /
